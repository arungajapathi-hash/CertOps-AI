...
## Changelog
- 2026-06-12 (Latest): **HACKATHON DEMO QUALITY FIXES (4/4)** — ISSUE 1: Created backend/plugins/resource_finder.py with verified URL validation. All learning resources now verified against trusted domains (MS Learn Catalog API, GitHub, YouTube, Tech Community). Links use validate_url() to prevent 404s, fallback to search URLs. All resources rendered as clickable HTML with target='_blank'. ISSUE 2: Created frontend/components/verdict_explainer.py showing transparent verdict reasoning with 4-step evidence chain: (1) Assessment score vs threshold with gap analysis, (2) Topic breakdown showing weak topics with exact scores and correct counts, (3) Council concerns listing agents that flagged issues, (4) What needs improvement with fallback logic. ISSUE 3: Added run_adaptive_loop() and _adapt_for_weak_topics() to Orchestrator. Implements proper adaptive loop: assessment → if FAIL → Socratic coach → regenerate study plan → retry (max 3 iterations). Each iteration records changes made. Added POST /adaptive endpoint with AdaptiveRequest model. ISSUE 4: Redesigned assessment page with render_interactive_exam() and render_exam_results(). One question at a time with timer, progress bar, immediate feedback after each answer, question navigator dots, final review showing all questions with correct answer + explanation, resources linked per wrong topic. On FAIL shows "Regenerate Learning Path" button calling /adaptive endpoint. All 4 issues resolved for hackathon demo.
